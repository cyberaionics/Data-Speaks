...
```python
from pipeline.02_segmentation.segmentation import segment_raw, segmentation_dataframe

# Segment preprocessed raw EEG using the real public callable
...
print(f"Windows shape: {seg_result.windows.shape}")  # (1799, 17, 1024)
print(f"Number of metadata entries: {len(seg_result.metadata)}")

# Convert metadata to pandas DataFrame
meta_df = segmentation_dataframe(seg_result)
print(meta_df.head())
```
