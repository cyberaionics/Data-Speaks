...
spec = importlib.util.spec_from_file_location(
    "frequency_domain_features_test_module",
    Path(__file__).with_name("features.py"),
)
if spec is None or spec.loader is None:
    raise ImportError("Could not load frequency-domain features module")
features_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(features_module)

extract_frequency_domain_features = features_module.extract_frequency_domain_features
flatten_frequency_domain_features = features_module.flatten_frequency_domain_features
