...
INPUT_DIR = Path("data/processed/pipeline_D")
OUTPUT_PATH = Path("results/benchmarks/features_pipeline_D_chb02_full.csv")
...
for recording_number, input_path in enumerate(processed_files, start=1):
    ...
    for window_index, window_info in enumerate(windows):
        ...
        rows.append({"recording_id": recording_id, "window_index": window_index, "start_sec": window_info["start_sec"], "end_sec": window_info["end_sec"], "label": window_info["label"], **features})
...
print("Pipeline D feature dataset complete")
print("Rows:", len(rows))
