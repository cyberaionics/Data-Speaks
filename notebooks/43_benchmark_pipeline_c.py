...
pipeline_c = load_module(
    "pipeline_c", "pipeline/01_preprocessing/candidate_algorithms/pipeline_c.py"
)
...
OUTPUT_DIR = Path("data/processed/pipeline_C")
...
config = pipeline_c.PipelineCConfig()
results = []
...
for index, input_path in enumerate(recordings, start=1):
    ...
    processed, provenance = pipeline_c.run_pipeline_c(raw, config=config, recording_id=recording_id)
    ...
    assert output_sfreq == config.target_sfreq
...
provenance_path = RESULTS_DIR / "pipeline_C_provenance.json"
with open(provenance_path, "w") as f:
    json.dump(results, f, indent=2, default=json_converter)

seizure_recordings = sum(bool(result["seizure_intervals"]) for result in results)
total_runtime = sum(result["runtime_sec"] for result in results)
print("Pipeline C benchmark complete")
print("Recordings:", len(results))
print("Seizure recordings:", seizure_recordings)
